...
REMOTE_ARTIFACTS="~/.hermes/workspace/projects/AutoResearchClaw/artifacts/"
LOCAL_ROOT="${LOCAL_ROOT:?Set LOCAL_ROOT to the local artifact sync directory}"

mkdir -p "$LOCAL_ROOT"
ssh -i "$KEY_FILE" "$REMOTE_HOST" "mkdir -p ~/.hermes/workspace/projects/AutoResearchClaw/artifacts"
