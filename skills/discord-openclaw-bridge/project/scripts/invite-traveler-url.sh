#!/usr/bin/env bash
set -euo pipefail
CLIENT_ID="${1:-${DISCORD_TRAVELER_CLIENT_ID:-}}"
if [ -z "$CLIENT_ID" ] && [ -f "$(dirname "$0")/../.env" ]; then
  CLIENT_ID="$(grep -E '^DISCORD_TRAVELER_CLIENT_ID=' "$(dirname "$0")/../.env" | tail -1 | cut -d= -f2- || true)"
fi
if [ -z "$CLIENT_ID" ]; then
  echo "usage: $0 CLIENT_ID" >&2
  echo "or set DISCORD_TRAVELER_CLIENT_ID in project/.env" >&2
  exit 2
fi
