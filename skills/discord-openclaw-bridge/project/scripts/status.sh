...
echo "== safe config =="
if [ -f .env ]; then
  grep -E '^(DISCORD_CLIENT_ID|DISCORD_MINER_CLIENT_ID|DISCORD_GUARD_CLIENT_ID|DISCORD_GUILD_ID|DISCORD_ALLOWED_CHANNEL_ID|DISCORD_MINER_CHANNEL_ID|DISCORD_OPS_REPORT_CHANNEL_ID|OPENCLAW_BASE_URL|OPENCLAW_MODEL|DISCORD_ENABLE_MENTION_RESPONSES|DISCORD_MINER_ENABLE_CHANNEL_COLLECTION|DISCORD_BRIEFING_SOURCE|JIPHYEONJEON_MINER_INTAKE_PATH|JIPHYEONJEON_MINER_REVIEW_QUEUE_PATH|JIPHYEONJEON_MINER_DECISIONS_PATH|MINER_SEEDS_STATUS_PATH)=' .env || true
