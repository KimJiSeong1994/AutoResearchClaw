...
class OpenClawDiscordBot(discord.Client):
    ...
    async def on_ready(self) -> None:
        LOG.info("ready user=%s guild=%s channel=%s", self.user, self.config.guild_id, self.config.allowed_channel_id)
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.author.bot or message.guild is None:
            return
        if message.guild.id != self.config.guild_id or message.channel.id != self.config.allowed_channel_id:
            return
