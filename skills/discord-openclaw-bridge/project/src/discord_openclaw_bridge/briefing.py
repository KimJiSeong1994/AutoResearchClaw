...
def render_briefing(source_path: Path, *, max_chars: int = 1800) -> Briefing:
    ...
    is_weekly_soul = "report_type: weekly-soul-trends" in raw or "# Weekly research trends" in text

    conclusion = _first_paragraph(_section(text, "1. 한 줄 결론"))
    if not conclusion and is_weekly_soul:
        conclusion = _first_paragraph(_section(text, "At a glance"))

    trend_block = _section(text, "2. 핵심 동향 5가지")
    if not trend_block and is_weekly_soul:
    ...
    for line in apply_block.splitlines():
        ...
        if len(actions) >= 3:
            break
    if not actions and is_weekly_soul:
    ...
    header = "**집현전-Claw 관심논문·SOUL 기반 연구 동향 브리핑**" if is_weekly_soul else "**집현전-Claw 업계 동향 브리핑**"
    lines = [
        header,
        f"_source: {source_path.name}_",
    ]
    if is_weekly_soul:
        lines.append("_basis: EC2 paper-recommender profile/SOUL + 집현전 검색 evidence_")
    lines += [
        "",
        f"**{title}**",
    ]
    if conclusion:
        if is_weekly_soul and len(conclusion) > 420:
            conclusion = conclusion[:397].rstrip(" ,.") + "…"
        lines += ["", f"**한 줄 결론**\n{conclusion}"]
    if trends:
        trend_heading = "**SOUL 기반 핵심 연구/기술 클러스터**" if is_weekly_soul else "**AI 업계 핵심 동향**"
        lines += ["", trend_heading]
    ...
    if actions:
        action_heading = "**우선 읽을 논문/근거**" if is_weekly_soul else "**연구·제품 관점 시사점**"
        lines += ["", action_heading]
        shown_actions = actions[:2] if is_weekly_soul else actions
        if is_weekly_soul:
            shown_actions = [a.split(" — ", 1)[0] for a in shown_actions]
        lines += [f"- {item}" for item in shown_actions]
    tail = "자세한 SOUL 기반 연구 동향 원문은 PaperWiki/weekly report에 보존되어 있습니다." if is_weekly_soul else "자세한 업계 동향 원문은 PaperWiki의 AI Newsletter 페이지에 발행되어 있습니다."
    lines += ["", tail]
