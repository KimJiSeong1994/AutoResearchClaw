...
def render_card_news_messages(payload: dict[str, Any], *, max_cards: int = 8) -> list[str]:
    ...
    messages = [
        "\n".join(
            [
                f"**{CARD_NEWS_TITLE} — AI Research**",
                f"발행일: `{run_date}`",
                "연구자가 빠르게 기술 변화와 연구 동향을 훑어보도록 핵심 카드만 선별했습니다.",
                f"선별 카드: {len(cards)}개 / 수집 항목: {len(items)}개",
                "구성: 제목 → 토픽 → 3줄 요약 → 기술 포인트 → 의미 → 근거 → 출처",
            ]
        )
    ]
    for item in cards:
        ...
        if isinstance(reasons, list):
            reason_text = _clean(", ".join(str(reason) for reason in reasons[:4]) or "fallback", limit=100)
        else:
        ...
        messages.append(
            "\n".join(
                [
                    ...
                    "",
                    "**기술 포인트**",
                    f"- 방법/차이: {_clean(summary[1], limit=120)}",
                    "- 한계/검증: 공개 원문 기준으로 평가 지표와 적용 범위를 후속 확인",
                    "",
                    "**의미**",
                    _clean(summary[2], limit=180),
                    "",
                    "**근거**",
                    f"분류 근거: {reason_text}",
                    "",
...
async def run() -> None:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        channel_type = int(channel_response.json().get("type", 0))
        purged = 0
