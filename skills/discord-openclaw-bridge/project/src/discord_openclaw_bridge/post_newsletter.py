...
async def run() -> None:
    ...
    body = _load_message(source, max_chars=max_chars)

    url = f"https://discord.com/api/v10/channels/{channel_id}/messages"
    headers = {"Authorization": f"Bot {token}"}
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(url, headers=headers, json={"content": body})
        response.raise_for_status()
    print(f"posted newsletter briefing to channel={channel_id} source={source}")
