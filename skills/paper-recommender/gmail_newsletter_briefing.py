...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--token", default=os.environ.get("GMAIL_TOKEN_PATH", str(Path.home() / ".hermes" / "workspace" / "secrets" / "gmail-token.json")))
    ...
    p.add_argument("--wiki-root", default=os.environ.get("NEWSLETTER_WIKI_ROOT", str(Path.home() / ".hermes" / "workspace" / "wiki")))
    p.add_argument("--briefing-path", default=os.environ.get("NEWSLETTER_REPORT_PATH", str(Path.home() / ".hermes" / "workspace" / "reports" / "newsletter-briefing-latest.md")))
