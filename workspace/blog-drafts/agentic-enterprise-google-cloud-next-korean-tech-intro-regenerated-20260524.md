...
slug: "agentic-enterprise-data-governance-evidence-regenerated-20260524"
...
대표 이미지 설명: 여러 AI agent가 기업 데이터 지도 위에서 움직이고, 각 agent의 경로가 권한 카드·근거 URL·감사 로그로 연결되는 추상 기술 일러스트. 로고, 읽을 수 있는 텍스트, 실존 인물은 넣지 않는다.
...
### 1. 관찰: agent는 제품 기능이 아니라 운영 모델이 되고 있다
...
### 2. 메커니즘: 실행 agent에는 데이터 맥락과 행동 표면이 필요하다
...
### 4. 반론: Google 중심 신호를 시장 전체 결론으로 과장하면 안 된다
...
1. **Agent registry와 identity**: 어떤 agent가 승인되었고 누가 책임지는가.
2. **Data catalog와 semantic layer**: agent가 데이터 의미를 어떻게 이해하는가.
3. **Action surface와 permission**: 어떤 API·도구 호출이 허용되는가.
4. **Evidence trail**: agent가 만든 요약과 결정의 원문 URL, 수집 경로, 수집일이 남는가.
5. **Human stop point**: 사람이 언제 중단하고 수정할 수 있는가.
